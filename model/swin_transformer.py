...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim  # 输入维数（嵌入补丁维数*2）^层数
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False)  # 线性层
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape  # (1,4096,96)
        assert L == H * W, "input feature has wrong size"
        assert H % 2 == 0 and W % 2 == 0, f"x size ({H}*{W}) are not even."

        x = x.view(B, H, W, C)  # 重新分为(1,64,64,96)
        # 将2、3维分为32*32的四部分
        x0 = x[:, 0::2, 0::2, :]  # B H/2 W/2 C
        x1 = x[:, 1::2, 0::2, :]  # B H/2 W/2 C
        x2 = x[:, 0::2, 1::2, :]  # B H/2 W/2 C
        x3 = x[:, 1::2, 1::2, :]  # B H/2 W/2 C 均为(1,32,32,96)
        x = torch.cat([x0, x1, x2, x3], -1)  # B H/2 W/2 4*C 合并四块的最后一维->(1,32,32,96*4)
        x = x.view(B, -1, 4 * C)  # B H/2*W/2 4*C  重新分为(1,-1,96*4)->(1,1024,384)
        # x(1,1024,384)
        x = self.norm(x)  # 规范化
        x = self.reduction(x)  # 全连接层降维 384->192
        # x(1,1024,192)
        return x
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # input_resolution = (patches_resolution[0] // (2 ** i_layer), patches_resolution[1] // (2 ** i_layer)),  # 输入分辨率
