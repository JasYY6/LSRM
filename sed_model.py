...
class SEDWrapper(pl.LightningModule):
    def __init__(self, sed_model, config, dataset):
        ...
        #self.loss_func = FocalLoss(num_classes=config.classes_num, alpha=[0.22, 0.27, 0.27, 0.24], gamma=2)
        self.loss_func = FocalLoss(num_classes=config.classes_num, alpha=0.5, gamma=2)
    ...
    def confusion_matrix(self, gather_pred, gather_target):
        # 0-asthma-50  1-COPD-136  2-normal-194  3-pneumonia-100   共480
        pred = []
        for i in range(len(gather_pred)):
            pred.append(np.argmax(gather_pred[i]))
        matrix = confusion_matrix(gather_target, pred)
        ...
        print("----------------------------")
        print("asthma:")
        print("\t查准率：%.2f" % ((matrix[0][0] / (matrix[0][0] + matrix[1][0] + matrix[2][0] + matrix[3][0])) * 100))
        print("\t召回率：%.2f" % ((matrix[0][0] / (matrix[0][0] + matrix[0][1] + matrix[0][2] + matrix[0][3])) * 100))
        print("COPD:")
        print("\t查准率：%.2f" % ((matrix[1][1] / (matrix[0][1] + matrix[1][1] + matrix[2][1] + matrix[3][1])) * 100))
        print("\t召回率：%.2f" % ((matrix[1][1] / (matrix[1][0] + matrix[1][1] + matrix[1][2] + matrix[1][3])) * 100))
        print("normal:")
        print("\t查准率：%.2f" % ((matrix[2][2] / (matrix[0][2] + matrix[1][2] + matrix[2][2] + matrix[3][2])) * 100))
        print("\t召回率：%.2f" % ((matrix[2][2] / (matrix[2][0] + matrix[2][1] + matrix[2][2] + matrix[2][3])) * 100))
        print("pneumonia:")
        print("\t查准率：%.2f" % ((matrix[3][3] / (matrix[0][3] + matrix[1][3] + matrix[2][3] + matrix[3][3])) * 100))
        print("\t召回率：%.2f" % ((matrix[3][3] / (matrix[3][0] + matrix[3][1] + matrix[3][2] + matrix[3][3])) * 100))
        print("----------------------------")
        print("Acc：%.2f" % (((matrix[0][0] + matrix[1][1] + matrix[2][2] + matrix[3][3]) / sum) * 100))
        print("----------------------------")
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        if torch.cuda.device_count() > 1:
            ...
            if dist.get_rank() == 0:
                ...
                self.confusion_matrix(gather_pred, gather_target)
                print(self.device_type, dist.get_world_size(), metric_dict, flush=True)

            if self.config.dataset_type == "audioset":
                self.log("mAP", metric_dict["mAP"] * float(dist.get_world_size()), on_epoch=True, prog_bar=True,
                         sync_dist=True)
                self.log("mAUC", metric_dict["mAUC"] * float(dist.get_world_size()), on_epoch=True, prog_bar=True,
                         sync_dist=True)
                self.log("dprime", metric_dict["dprime"] * float(dist.get_world_size()), on_epoch=True, prog_bar=True,
                         sync_dist=True)
            else:
                print("测试用：")
                print(metric_dict["acc"] * float(dist.get_world_size()))
                self.log("acc", metric_dict["acc"] * float(dist.get_world_size()), on_epoch=True, prog_bar=True,
                         sync_dist=True)
                print("测试用：")
